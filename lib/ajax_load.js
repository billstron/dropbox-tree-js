(function($) {
	$.jstree.plugin("ajax_data", {
		__init : function() {
			var s = this._get_settings().ajax_data;
		...
		_fn : {
			...
			_is_loaded : function(obj) {
				console.log("_is_loaded:", obj);
				var s = this._get_settings().ajax_data;
				obj = this._get_node(obj);
				return obj == -1 || !obj || (!s.ajax && !s.progressive_render && !$.isFunction(s.data)) || obj.is(".jstree-open, .jstree-leaf")
			...
			load_node_json : function(obj, s_call, e_call) {
				console.log("load_node_json:", obj);
				var s = this.get_settings().ajax_data, d, error_func = function() {
				}, success_func = function() {
				};
				obj = this._get_node(obj);

				if (obj && obj !== -1 && (s.progressive_render || s.progressive_unload) && !obj.is(".jstree-open, .jstree-leaf")
				...
				case (!s.data && !!s.ajax) || (!!s.data && !!s.ajax && obj && obj !== -1):
					error_func = function(x, t, e) {
						var ef = this.get_settings().ajax_data.ajax.error;
						if (ef) {
							ef.call(this, x, t, e);
						}
						if (obj != -1 && obj.length) {
					...
					success_func = function(d, t, x) {
						var sf = this.get_settings().ajax_data.ajax.success;
						if (sf) {
			...
			_parse_json : function(js, obj, is_callback) {
				console.log("parser_json:", obj);
				var d = false, p = this._get_settings(), s = p.ajax_data, t = p.core.html_titles, tmp, i, j, ul1, ul2;
			...
			get_json : function(obj, li_attr, a_attr, is_callback) {
				...
				obj.each(function() {
					...
					a.each(function() {
						t = $(this);
						if (a_attr.length || $.inArray("languages", s.plugins) !== -1 || t.children("ins").get(0).style.backgroundImage.length
