(function() {
	...
	function DBoxTree(needle, client) {
		...
		this.onRemove = function onRemove(name, node, data) {
			...
			client.remove(path, function(err) {
				if (err) {
					self.writeMessage("Dropbox could not remove.");
					return $.jstree.rollback(data.rlbk);
				}
		...
		this.onPaste = function onPaste(data) {
			...
			var toCopy = data.rslt.nodes;
			
			var child = getChild(needle, parent, toCopy.attr("id"));
			
			var name = $(needle).jstree("get_text", child);
			
			var children = getChildrenNames(needle, parent);
			var iName = children.indexOf(name);
			children.splice(iName, 1);
	...
	function getChild(needle, parent, id) {
		var child = null;
		var children = $.jstree._reference(needle)._get_children(parent);
		for ( var i = 0; i < children.length; i++) {
